...
from modules.dns import DNSChecker
from modules.social import SocialChecker
from modules.whois import WhoisChecker
from utils.user_agent import getRandomUserAgent
from utils.file_operations import isFile, getLinesFromFile
from export.file_operations import createSaveDirectory
from export.csv_exporter import saveToCsv
from export.pdf_exporter import saveToPdf
...
def initiate():
    ...
    parser = argparse.ArgumentParser(
        ...
        description="An OSINT tool to search for accounts by username and email on social networks and other platforms.",
    )
    parser.add_argument(
        "-u",
        "--username",
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=str,
        help="One or more emails to search.",
    )
    parser.add_argument(
        "-ef",
        "--email-file",
...
def main():
    ...
    if config.username:
        ...
        for user in config.username:
            ...
            if config.dump or config.csv or config.pdf:
                createSaveDirectory()
            UsernameChecker(config.currentUser).run()
    ...
    if config.email:
        for email in config.email:
            ...
            if config.dump or config.csv or config.pdf:
                createSaveDirectory()
            EmailChecker(config.currentEmail).run()
